...
namespace MissionPlanner
{
    public class Grid
    {
        ...
        public static List<PointLatLngAlt> CreateGrid(List<PointLatLngAlt> polygon, double altitude, double distance, double spacing, double angle, double overshoot1,double overshoot2, StartPosition startpos, bool shutter, float minLaneSeparation, float leadin = 0, double adjust = 0)
        {
            ...
            while (grid.Count > 0)
            {
                ...
                if (closest.p1.GetDistance(lastpnt) < closest.p2.GetDistance(lastpnt))
                {
                    ...
                    if (grid.Count > 1)
                    {
                        linelatlng tempnext = findClosestLine(closest.p2, grid, minLaneSeparationINMeters, angle);
                        utmpos temppnt = tempnext.p1;
                        if (tempnext.p1.GetDistance(closest.p2) < tempnext.p2.GetDistance(closest.p2))
                            temppnt = tempnext.p2;
                        //在这里浪费了好多时间啊mark一下
                    }
                    ...
                    linelatlng tempclosest = closest;
                    grid.Remove(closest);
                    if (grid.Count == 0)
                        break;
        ...
        static bool TurnRight(linelatlng templine, linelatlng nextline, utmpos endpnt)
        {
            bool turnright = true;
