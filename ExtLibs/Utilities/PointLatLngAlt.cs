...
namespace MissionPlanner.Utilities
{
    ...
    public class PointLatLngAlt: IComparable
    {
        ...
        public int FarthestPointNo = 0;
        public double MinArea = 0;
        public double MaxWidth = 0;
        public double MaxLengthInD1 = 0;
        public double MaxLengthInD2 = 0;
        public double MaxLength = 0;
        public int FarthestPointInDirection1 = 0;
        public int FarthestPointInDirection2 = 0;
        ...
        public double Cross(PointLatLngAlt p2, PointLatLngAlt p3)//计算向量p2p1与向量p3p1的叉乘
        {
            double Dis12 = this.GetDistance(p2);
            double Dis13 = this.GetDistance(p3);
            double Brg213 = this.GetBearing(p2) - this.GetBearing(p3);
            return Dis12 * Dis13 * Math.Sin(Math.PI*Brg213/180);
        }

        public double ClockwiseAngle(PointLatLngAlt p2, PointLatLngAlt p3)//计算p2p1p3顺时针夹角
        {
            if (this.GetBearing(p2) > this.GetBearing(p3))
                return (360 - this.GetBearing(p2) + this.GetBearing(p3));
            else return (this.GetBearing(p3) - this.GetBearing(p2));
        }

        public double Dot(PointLatLngAlt p2, PointLatLngAlt p3)//计算向量p1p2与p1p3的点积
        {
            double Dis12 = this.GetDistance(p2);
            double Dis13 = this.GetDistance(p3);
            double Brg213 = this.GetBearing(p2) - this.GetBearing(p3);
            return Dis12 * Dis13 * Math.Cos(Math.PI * Brg213 / 180);
        }
    }
}
