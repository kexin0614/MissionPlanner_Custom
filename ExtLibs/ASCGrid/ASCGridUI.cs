...
namespace MissionPlanner
{
    public partial class ASCGridUI : Form
    {
        ...
        void DrawPolygon()
        {
            ...
            ascpoly.Fill = new SolidBrush(Color.FromArgb(50,Color.Red));
        ...
        public void CreateXml()
        {
            ...
            if (picturesInfo != null)
            {
                ...
                try
                {
                    ascxmldoc.Save(TXT_jpgdir.Text + @"\AutogeneratedProj.p4d");
                    MessageBox.Show("成功生成工程文件到图片文件夹");
                }
                catch(Exception ex)
                {
                    MessageBox.Show("失败！请检查文件夹路径");
                    throw ex;
                }
            ...
            else
            {
                ...
                {
                    ascxmldoc.Save(TXT_jpgdir.Text + @"\AutogeneratedProj.p4d");
                    MessageBox.Show("已生成工程文件到图片文件夹，但未包含图像信息");
                }
                catch (Exception ex)
                {
                    MessageBox.Show("失败！请检查文件夹路径");
        ...
        {
            using (var form = new LogDownloadMavLink())
            {
                form.ShowDialog();
            }
        ...
        private void BUT_CreateP4D_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                ...
                {
                    Process p = Process.Start(objResult.ToString() + "pix4dmapper.exe", TXT_jpgdir.Text + @"\AutogeneratedProj.p4d");
                }
            }
            catch
            {
                ...
                if (File.Exists(openFileDialog1.FileName))
                {
                    Process p = Process.Start(openFileDialog1.FileName, TXT_jpgdir.Text + @"\AutogeneratedProj.p4d");
                    // TXT_jpgdir.Text = Path.GetDirectoryName(TXT_logfile.Text);
                }
            }
