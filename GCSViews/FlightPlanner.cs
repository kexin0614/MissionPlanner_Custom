...
namespace MissionPlanner.GCSViews
{
    public partial class FlightPlanner : MyUserControl, IDeactivate, IActivate
    {
        ...
        PointLatLngAlt MyRectangle2;
        PointLatLngAlt MyRectangle3;
        ...
        public List<PointLatLngAlt> NewPolygonPoints = new List<PointLatLngAlt>();//凸多边形点
        //public List<PointLatLngAlt> MyRectangle = new List<PointLatLngAlt>();//外包矩形
        ...
        public FlightPlanner()
        {
            ...
            //是否处于根据多边形区域绘制自动航点功能中
            polygonpathmode = false;
...
Column 1: Field type (RALLY is the only one at the moment -- may have RALLY_LAND in the future)
 ...
 Column 7: Flags (just 0 for now, also future use).
        ...
        //多边形点转换成PointLatLngAlt类
        private void ChangeAll2PointLatLngAlt (int n)
        {
            if (NewPolygonPoints != null)
                NewPolygonPoints.Clear();
            for (int i = 0; i < n; i++)
            {
                NewPolygonPoints.Add(new PointLatLngAlt(drawnpolygon.Points[i].Lat, drawnpolygon.Points[i].Lng));
            }
        }
        //判断图形顺时针与逆时针，true为顺，false为逆
        private bool Direction(int n)
        {
           double AngleSum = 0;
            for (int i = 2; i < n; i++)
            {
                AngleSum = AngleSum + NewPolygonPoints[i - 1].ClockwiseAngle(NewPolygonPoints[i - 2], NewPolygonPoints[i]);
            }
            AngleSum = AngleSum + NewPolygonPoints[0].ClockwiseAngle(NewPolygonPoints[n - 1], NewPolygonPoints[1]);
            AngleSum = AngleSum + NewPolygonPoints[n-1].ClockwiseAngle(NewPolygonPoints[n - 2], NewPolygonPoints[0]);
            if(AngleSum - (n-2)*180 <= 0.01)
                return false;
            else return true;
        }
        //删除构成凹多边形的NewPolygonPoints点
        private void ConvexPolygon(int n)
        {
            int tempn;
            do
            {
                ...
                for (int i = 2; i < n; i++)
                {
                    if (NewPolygonPoints[i - 2].Cross(NewPolygonPoints[i - 1], NewPolygonPoints[i]) <= 0 && i >= 2)
                    {
                        NewPolygonPoints.RemoveAt(i - 1);
                        n--;
                    }
                }
                if (NewPolygonPoints[n - 2].Cross(NewPolygonPoints[n - 1], NewPolygonPoints[0]) <= 0)
                {
                    NewPolygonPoints.RemoveAt(n - 1);
                    n--;
                }
                if (NewPolygonPoints[n - 1].Cross(NewPolygonPoints[0], NewPolygonPoints[1]) <= 0)
                {
                    NewPolygonPoints.RemoveAt(0);
                    n--;
                }
            } while (tempn != n);
        }
        ...
        //确定最小外包矩形
        private void FindSmallestRectangle(int n)
        {
            int TempFarthestPoint = 0;
            int TempFarthestInD1 = 0;
            int TempFarthestInD2 = 0;
            double TempMaxWidth = 0;
            double TempMaxLengthInD1 = 0;
            double TempMaxLengthInD2 = 0;
            
            for (int i = 0; i < n; i++)
            {
                int next = (i + 1) % n;
                for (int j = 0; j < n;j++ )//确定距离i和next直线最远的点
                {
                    if (NewPolygonPoints[i].Cross(NewPolygonPoints[next], NewPolygonPoints[j]) / NewPolygonPoints[i].GetDistance(NewPolygonPoints[next]) >= TempMaxWidth)
                    {
                        TempMaxWidth = NewPolygonPoints[i].Cross(NewPolygonPoints[next], NewPolygonPoints[j]) / NewPolygonPoints[i].GetDistance(NewPolygonPoints[next]);
                        TempFarthestPoint = j;
                    }     
                }
                NewPolygonPoints[i].MaxWidth = TempMaxWidth; ;
                NewPolygonPoints[i].FarthestPointNo = TempFarthestPoint;
                
                for(int j = 0; j < n; j++ )//确定D1方向最远的点
                {
                    if (NewPolygonPoints[i].Dot(NewPolygonPoints[next], NewPolygonPoints[j]) / NewPolygonPoints[i].GetDistance(NewPolygonPoints[next]) >= TempMaxLengthInD1)
                    {
                        TempMaxLengthInD1 = NewPolygonPoints[i].Dot(NewPolygonPoints[next], NewPolygonPoints[j]) / NewPolygonPoints[i].GetDistance(NewPolygonPoints[next]);
                        TempFarthestInD1 = j;
                    }
                }
                NewPolygonPoints[i].FarthestPointInDirection1 = TempFarthestInD1;
                NewPolygonPoints[i].MaxLengthInD1 = TempMaxLengthInD1;

                for (int j = 0; j < n; j++)//确定D2方向最远的点
                {
                    if (NewPolygonPoints[i].Dot(NewPolygonPoints[next], NewPolygonPoints[j]) / NewPolygonPoints[i].GetDistance(NewPolygonPoints[next]) <= TempMaxLengthInD2)
                    {
                        TempMaxLengthInD2 = NewPolygonPoints[i].Dot(NewPolygonPoints[next], NewPolygonPoints[j]) / NewPolygonPoints[i].GetDistance(NewPolygonPoints[next]);
                        TempFarthestInD2 = j;
                    }
                }
                NewPolygonPoints[i].FarthestPointInDirection2 = TempFarthestInD2;
                NewPolygonPoints[i].MaxLengthInD2 = Math.Abs(TempMaxLengthInD2);

                NewPolygonPoints[i].MaxLength = NewPolygonPoints[i].MaxLengthInD1 + NewPolygonPoints[i].MaxLengthInD2;
                NewPolygonPoints[i].MinArea = NewPolygonPoints[i].MaxLength * NewPolygonPoints[i].MaxWidth;
            ...
            double TempMinArea = NewPolygonPoints[0].MinArea;
            int smalli=0;
            int tempsmalli = 0;
            
            for (tempsmalli = 0; tempsmalli < n; tempsmalli++)//找到面积最小的矩形
            {
                if (NewPolygonPoints[tempsmalli].MinArea < TempMinArea)
                {
                    TempMinArea = NewPolygonPoints[tempsmalli].MinArea;
                    smalli=tempsmalli;
                }
            }

            MyRectangle0 = NewPolygonPoints[smalli];
            MyRectangle0 = MyRectangle0.newpos(NewPolygonPoints[smalli].GetBearing(NewPolygonPoints[(smalli + 1) % n]), NewPolygonPoints[smalli].MaxLengthInD1);
           
            double Bearing1;
            
            if(NewPolygonPoints[smalli].GetBearing(NewPolygonPoints[(smalli+1)%n])>90)
                Bearing1 = NewPolygonPoints[smalli].GetBearing(NewPolygonPoints[(smalli+1)%n]) - 90;
            else Bearing1 = NewPolygonPoints[smalli].GetBearing(NewPolygonPoints[(smalli+1)%n]) + 270;

            MyRectangle1 = MyRectangle0;
            MyRectangle1 = MyRectangle1.newpos(Bearing1, NewPolygonPoints[smalli].MaxWidth);

            double Bearing2;
            if (MyRectangle0.GetBearing(MyRectangle1) > 90)
                Bearing2 = MyRectangle0.GetBearing(MyRectangle1) - 90;
            else Bearing2 = MyRectangle0.GetBearing(MyRectangle1) + 270;

            MyRectangle2 = MyRectangle1;
            MyRectangle2 = MyRectangle2.newpos(Bearing2, NewPolygonPoints[smalli].MaxLength);

            double Bearing3;
            if (Bearing1 > 180)
                Bearing3 = Bearing1 - 180;
            else Bearing3 = 180 - Bearing1;

            MyRectangle3 = MyRectangle2;
            MyRectangle3 = MyRectangle3.newpos(Bearing3, NewPolygonPoints[smalli].MaxWidth);

            //if (NewPolygonPoints[smalli].MaxWidth <= NewPolygonPoints[smalli].MaxLength)
            
                PointLatLngAlt TempPnt = MyRectangle0;
                MyRectangle0 = MyRectangle1;
                MyRectangle1 = MyRectangle2;
                MyRectangle2 = MyRectangle3;
                MyRectangle3 = TempPnt;
        ...
        private void myButton1_Click(object sender, EventArgs e)
        {
            ...
            polygonpathmode = true;
            if (!(drawnpolygon.Points.Count >= 3))
            {
                CustomMessageBox.Show("请设置多边形点。");
                polygonpathmode = false;
                return;
            }
            if (pointlist.Count >= 2)
            {
                if (CustomMessageBox.Show("此操作将清空已设置的其他航点，确认？", "将清空……",
                            MessageBoxButtons.OKCancel) != DialogResult.OK)
                {
                    polygonpathmode = false;
                    return;
            ...
            ChangeAll2PointLatLngAlt(drawnpolygon.Points.Count);

            if (Direction(drawnpolygon.Points.Count))
                NewPolygonPoints.Reverse();

            if (drawnpolygon.Points.Count>=5)
                ConvexPolygon(drawnpolygon.Points.Count);

            FindSmallestRectangle(NewPolygonPoints.Count);

            double brg1 = MyRectangle0.GetBearing(MyRectangle1);
            double brg2 = MyRectangle0.GetBearing(MyRectangle3);
            double brg3 = MyRectangle1.GetBearing(MyRectangle2);
            double intvl = 100;
            bool tmp = true;

            PointLatLngAlt cur = new PointLatLngAlt(MyRectangle0);
            PointLatLngAlt cur2 = new PointLatLngAlt(MyRectangle1);
            while (MyRectangle0.GetDistance(cur) <= MyRectangle0.GetDistance(MyRectangle3) || MyRectangle1.GetDistance(cur2) <= MyRectangle1.GetDistance(MyRectangle2)) 

            {
                cur = cur.newpos(brg2, intvl);
                cur2 = cur2.newpos(brg3, intvl);
                if (tmp)
                {
                    AddWPToMap(cur.Lat , cur.Lng , 0);
                    AddWPToMap(cur2.Lat , cur2.Lng , 0);
                }
                else
                {
                    AddWPToMap(cur2.Lat, cur2.Lng, 0);
            ...
            //foreach (var p in drawnpolygon.Points)
            //{
            //    s += string.Format("{0}:Lat{1}, Lng{2}\n", i.ToString(), p.Lat.ToString(), p.Lng.ToString());
            //    i++;
            //}
            //MessageBox.Show(s);
        }
        ...
        private void myButton2_Click(object sender, EventArgs e)
        {
            ...
            {
                selectedrow = a;
                Commands.Rows.Insert(2 * selectedrow + 1, 1);
            }
            //在每一个航点后插入新航点
            for (int a = 0; a < count; a++)
            {
                selectedrow = a;
                Commands.Rows[2 * selectedrow + 1].Cells[Command.Index].Value = MAVLink.MAV_CMD.DO_DIGICAM_CONTROL.ToString();
                ChangeColumnHeader(MAVLink.MAV_CMD.DO_DIGICAM_CONTROL.ToString());
            }
            //将每个新插入的航点变为DO_DIGICAM_CONTROL类型航点
            writeKML();
            //郑可心最棒棒了科科
        }
